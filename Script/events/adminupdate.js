        ...
        switch (logMessageType) {
            case "log:thread-admins": {
                if (logMessageData.ADMIN_EVENT == "add_admin") {
                    ...
                    if (global.configModule[this.config.name].sendNoti) api.sendMessage(`»» NOTICE «« Update user ${logMessageData.TARGET_ID} -এই নাও তোমাকে এডমিন 👑 এর দায়িত্ব দিয়া হলো- আশা করি ভালো ভাবে দায়িত্ব পালন করবে 🥰 -সিফাত স্যারের পক্ষ থেকে তোমাকে অভিনন্দন জানানো হয়েছে💗-😁🫵🏾`, threadID, async (error, info) => {
                    ...
                    if (global.configModule[this.config.name].sendNoti) api.sendMessage(`»» NOTICE «« Update user ${logMessageData.TARGET_ID} তুমি এডমিন হওয়ার যোগ্য না \n তাই তোমাকে উপর থেকে এডমিন থেকে সরানো হইছে !🔑🥱|`, threadID, async (error, info) => {
                        if (global.configModule[this.config.name].autoUnsend) {
                            await new Promise(resolve => setTimeout(resolve, global.configModule[this.config.name].timeToUnsend * 1000));
                            return api.unsendMessage(info.messageID);
                        } else return;
